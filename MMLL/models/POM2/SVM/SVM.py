# -*- coding: utf-8 -*-
'''
Semiparametric Support Vector Machine model under POM2.
'''
...
from MMLL.models.POM2.Kmeans.Kmeans import Kmeans_Master, Kmeans_Worker
...
class SVM_Master(POM2_CommonML_Master):
    """
    This class implements SVM, run at Master node. It inherits from :class:`POM2_CommonML_Master`.
    """
    ...
    def __init__(self, comms, logger, verbose=False, NC=None, Nmaxiter=None, tolerance =None, sigma=None, C=None, NmaxiterGD=None, eta=None):
        ...
        self.name = 'POM2_SVM_Master'                             # Name
        self.iter = 0                                             # Number of iterations
        self.weights = np.zeros((self.num_centroids, 1))          # Initialize model weights
    ...
    def TakeAction_Master(self):
        ...
        if self.state_dict['CN'] == 'TRAIN_KMEANS':
            ...
            self.public_key = kmeans_master.public_key # Store the public key
        ...
        if self.state_dict['CN'] == 'UPDATE_MODEL':
            ...
            self.weights = self.weights - self.eta*gradients
        ...
        if self.state_dict['CN'] == 'COMPUTE_GRADIENTS':
            if self.iter == 0:
                ...
                self.weights = self.encrypt_list(list(self.weights)) # Convert unidimensional array to list to fix errors in encryption
...
class SVM_Worker(POM2_CommonML_Worker):
    '''
    Class implementing a semiparametric SVM, run at Worker node. It inherits from :class:`POM2_CommonML_Worker`.
    '''
    ...
    def __init__(self, master_address, comms, logger, verbose=False, Xtr_b=None, ytr=None):
        ...
        self.name = 'POM2_SVM_Worker'                               # Name
    ...
    def ProcessReceivedPacket_Worker(self, packet):
        ...
        if packet['action'] == 'COMPUTE_LOCAL_GRADIENTS':
            ...
            encrypted_gradients = np.asarray(self.encrypt_list(gradients))
            action = 'UPDATE_GRADIENTS'
            data = {'cost_function': cost_function, 'gradients': encrypted_gradients}
