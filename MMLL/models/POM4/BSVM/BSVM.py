# -*- coding: utf-8 -*-
'''
Budget Support Vector Machine under POM4
...
class BSVM_Master(Common_to_all_POMs):
    ...
    def create_FSM_master(self):
        ...
        states_master = [
            ...
            State(name='get_sign', on_enter=['while_get_sign'])
        ]
        ...
        transitions_master = [
            ...
            ['go_get_sign', 'waiting_order', 'get_sign'],
            ['done_get_sign', 'get_sign', 'waiting_order'],
        ...
        class FSM_master(object):
            ...
            def while_mult_AB(self, MLmodel, A_encr_dict=None, B_encr_dict=None):
                try:
                    ...
                    for waddr in A_encr_dict.keys():
                        N = A_encr_dict[waddr].shape[0]
                        MLmodel.A_bl_dict.update({waddr: np.random.uniform(0.1, 10, (N, 1))})
                        MLmodel.B_bl_dict.update({waddr: np.random.uniform(0.1, 10, (N, 1))})
                        A_encr_bl_dict.update({waddr: A_encr_dict[waddr] * MLmodel.A_bl_dict[waddr]})
                        B_encr_bl_dict.update({waddr: B_encr_dict[waddr] * MLmodel.B_bl_dict[waddr]})
                    
                    MLmodel.display('PROC_MASTER_END', verbose=False)

                    # We send blinded A and B 
                    data = {'A_encr_bl_dict': A_encr_bl_dict, 'B_encr_bl_dict': B_encr_bl_dict}
                    action = 'send_mult_AB'
                    packet = {'action': action, 'to': 'MLmodel', 'data': data, 'sender': MLmodel.master_address}
                    ...
                    MLmodel.comms.send(packet, MLmodel.send_to[MLmodel.cryptonode_address])
                    MLmodel.display(MLmodel.name + ' send_mult_AB to cryptonode')
                except:
                    raise
                    '''
                    print('ERROR AT while_mult_AB')
                    import code
                    code.interact(local=locals())
                    pass
                    '''
            ...
            def while_get_sign(self, MLmodel, A_encr_dict):
                try:
                    MLmodel.display('PROC_MASTER_START', verbose=False)
                    MLmodel.A_bl_dict = {}
                    A_encr_bl_dict = {}
                    ...
                    for waddr in A_encr_dict.keys():
                        N = A_encr_dict[waddr].shape[0]
                        MLmodel.A_bl_dict.update({waddr: np.random.normal(0, 10, (N, 1))})
                        A_encr_bl_dict.update({waddr: A_encr_dict[waddr] * MLmodel.A_bl_dict[waddr]})

                    MLmodel.display('PROC_MASTER_END', verbose=False)

                    action = 'ask_sign_bl'
                    data = {'A_encr_bl_dict': A_encr_bl_dict}
                    packet = {'action': action, 'data': data, 'to': 'CommonML', 'sender': MLmodel.master_address}
    ...
    def train_Master(self):
        ...
        # Computing Kcc
        Dcc = np.abs(c2 -2 * np.dot(self.C, self.C.T) + c2.T)
        Kcc = np.exp(-Dcc / 2 / (self.model.sigma**2))
        # Extended with bias
        Kcc_ = np.zeros((self.NC + 1, self.NC + 1))
        Kcc_[1:, 1:] = Kcc
        del Kcc
        ...
        while not self.stop_training:
            ...
            NPtotal = 0
            for waddr in self.selected_workers:
                self.y_encr_dict[waddr] = self.y_encr_dict[waddr].reshape((-1, 1))
                y = self.y_encr_dict[waddr]
                NPtotal += y.shape[0]
            ...
            # We compute ey
            self.FSMmaster.go_mult_AB(self, self.y_encr_dict, self.e_encr_dict) #Blinding takes place in here
            self.run_Master()
            # we obtain self.Kxc_bl_dict, the blinding values, we store them at the BX value.
            #self.ey_encr_dict = self.AB_encr_dict


            # We compute the sign
            self.FSMmaster.go_get_sign(self, self.ey_encr_dict) #Blinding takes place in here
            self.run_Master()
            #self.sign_dict

            self.display('PROC_MASTER_START', verbose=False)
            # Anulando errores con signo negativo en ey
            # contando signos negativos
            for waddr in self.sign_dict.keys():
                aux = self.sign_dict[waddr]
                #print(np.sum(aux <0))
                self.e_encr_dict[waddr] = self.e_encr_dict[waddr] * aux
    ...
    def ProcessReceivedPacket_Master(self, packet, sender):
        ...
        try:
            ...
            if packet['action'] == 'ACK_sent_AB_bl_encr_dict':
                #AB_bl_encr_dict = packet['data']['AB_bl_encr_dict']
                # Deblinding
                self.AB_encr_dict = {}
                for waddr in packet['data']['AB_bl_encr_dict'].keys():
                    AB_bl_encr = packet['data']['AB_bl_encr_dict'][waddr]
                    AB_encr = AB_bl_encr / self.A_bl_dict[waddr] / self.B_bl_dict[waddr]
                    self.AB_encr_dict.update({waddr: AB_encr})

                self.ey_encr_dict = dict(self.AB_encr_dict)
                del self.AB_encr_dict
                self.FSMmaster.done_mult_AB(self)

            if packet['action'] == 'ACK_compute_sign':
                self.sign_bl_dict = packet['data']['sign_bl_dict']
                
                # Deblinding
                self.sign_dict = {}
                for waddr in packet['data']['sign_bl_dict'].keys():
                    sign_bl = packet['data']['sign_bl_dict'][waddr]
                    signs = sign_bl * np.sign(self.A_bl_dict[waddr])
                    self.sign_dict.update({waddr: signs})
                
                self.FSMmaster.done_get_sign(self)
...
class BSVM_Worker(Common_to_all_POMs):
    ...
    def __init__(self, master_address, worker_address, model_type, comms, logger, verbose=True, Xtr_b=None, ytr=None, cryptonode_address=None):
        ...
        self.ytr = (2 * np.array(ytr).reshape(-1, 1) - 1).astype(float)
        self.NPtr = len(ytr)
        ...
        class FSM_worker(object):
            ...
            def while_compute_gradients(self, MLmodel, packet):
                try:
                    ...
                    #print('STOP AT while_compute_gradients')
                    #import code
                    #code.interact(local=locals())                   
...
#                 Crypto
#===============================================================
class BSVM_Crypto(Common_to_all_POMs):
    '''
