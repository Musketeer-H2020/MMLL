            ...
            def while_updating_w(self, MLmodel):
                ...
                try:
                    ...
                    if MLmodel.Xval is None:  # A validation set is not provided
                        ...
                        inc_w = np.linalg.norm(MLmodel.model.w - MLmodel.w_old) / np.linalg.norm(MLmodel.w_old)
                        ...
                        which1 = (MLmodel.yval * 2 - 1) == 1
                        which_1 = (MLmodel.yval * 2 - 1) == -1
                        NP1 = np.sum(which1)
                        NP_1 = np.sum(which_1)

                        #CE_val = []
                        ACC_val = []
                        mus = np.arange(-0.2, 10.0, 0.01)
                        ...
                        for mu in mus:
                            ...
                            aciertos = np.sign((MLmodel.yval * 2 - 1)) == np.sign(o_val - 0.5)
                        ...
                        mu_opt = mus[max_pos]
                        print('Optimal mu = %f, ACC val=%f' % (mu_opt, ACCval_opt))
                        del mus

                        MLmodel.w_old = MLmodel.model.w.copy()
              
                        if ACCval_opt > MLmodel.acc_val_old:  
                            message = 'Maxiter = %d, iter = %d, ACC val = %f' % (MLmodel.Nmaxiter, MLmodel.kiter, ACCval_opt)
                            print(message)
