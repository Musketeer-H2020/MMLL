        ...
        class FSM_master(object):
            ...
            def while_updating_w(self, MLmodel):
                ...
                try:
                    ...
                    MLmodel.acc_val_old = MLmodel.acc_val
                    MLmodel.acc_val = 0
                    ...
                    for cla in MLmodel.classes:
                        ...
                        if MLmodel.Xval is None:  # A validation set is not provided
                            ...
                            inc_w += np.linalg.norm(MLmodel.model.w_dict[cla] - MLmodel.w_old_dict[cla]) / np.linalg.norm(MLmodel.w_old_dict[cla])
                            del grad_acum
                            ...
                            for mu in mus:
                                ...
                                aciertos = np.sign(yval - 0.5) == np.sign(o_val - 0.5).reshape((-1, 1))
                            ...
                            MLmodel.acc_val += ACCval_opt

                            indices = np.array(range(len(mus)))[ACC_val == ACCval_opt]
                            max_pos = indices[0]  # first
                            mu_opt = mus[max_pos]
                            print('Optimal mu = %f for class=%s, ACC val=%f' % (mu_opt, cla, ACCval_opt))
                            MLmodel.model.w_dict[cla] = MLmodel.model.w_dict[cla] - mu_opt * grad_acum

                    MLmodel.acc_val = MLmodel.acc_val / len(MLmodel.classes)

                    # Checking stopping
                    if MLmodel.Xval is None:  # A validation set is not provided
                        # Stop if convergence is reached
                        if inc_w < MLmodel.conv_stop:
                            MLmodel.stop_training = True
                        message = 'Maxiter = %d, iter = %d, inc_w = %f' % (MLmodel.Nmaxiter, MLmodel.kiter, inc_w)
                        print(message)
                    else:
                        if MLmodel.acc_val > MLmodel.acc_val_old:  
                            message = 'Maxiter = %d, iter = %d, ACC val = %f' % (MLmodel.Nmaxiter, MLmodel.kiter, MLmodel.acc_val)
                            print(message)
                        else:
                            MLmodel.stop_training = True
                            # We retain the last weight values
                            MLmodel.model.w_dict = dict(MLmodel.w_old_dict)
