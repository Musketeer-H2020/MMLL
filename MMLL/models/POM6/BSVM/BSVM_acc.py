    ...
    def train_Master(self):
        ...
        while not self.stop_training:
            ...
            if self.Xval is None:  # A validation set is not provided
                ...
                if ACCval_opt > self.acc_val_old:  
                    message = 'Maxiter = %d, iter = %d, ACC val = %f' % (self.Nmaxiter, self.kiter, ACCval_opt)
                    print(message)          
                    self.model.w = (1.0 - landa_opt) * self.w_old + landa_opt * self.model.w
                    self.acc_val = ACCval_opt
                else:
                    self.stop_training = True
